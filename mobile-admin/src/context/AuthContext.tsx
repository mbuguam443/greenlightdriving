...
export const STAFF_ROLES = ['SUPER_ADMIN', 'MANAGER', 'RECEPTIONIST', 'ACCOUNTANT', 'INSTRUCTOR'];
...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const login = async (email: string, password: string) => {
    ...
    if (!STAFF_ROLES.includes(data.user.role)) {
      throw new Error('This account is not a staff account. Use the student app instead.');
    }
  ...
  const logout = async () => {
    await clearAuth();
    setUser(null);
    await AsyncStorage.removeItem('gla_refresh_token');
  };
